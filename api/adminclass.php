...
class admin
{
	...
	public function getuser($mysqli, $idupd)
	{
		...
		if ($mysqli->affected_rows > 0) {
			...
			$detailrecords['uncleared_report'] = strip_tags($row->uncleared_report);
			...
			$detailrecords['reportmodule_interest'] = strip_tags($row->reportmodule_interest);
			$detailrecords['interest_ledger_report'] = strip_tags($row->interest_ledger_report);
			$detailrecords['interest_loan_issue_report'] = strip_tags($row->interest_loan_issue_report);
			$detailrecords['interest_collection_report'] = strip_tags($row->interest_collection_report);
			$detailrecords['interest_balance_report'] = strip_tags($row->interest_balance_report);
			$detailrecords['interest_closed_report'] = strip_tags($row->interest_closed_report);
	...
	public function addAreaMappingLine($mysqli, $userid)
	{
		...
		$insertQry = "INSERT INTO area_line_mapping(line_name,  company_id, branch_id, cus_count , loan_count, insert_login_id, created_date)
		VALUES('" . strip_tags($line_name) . "', '" . strip_tags($company_id) . "','" . strip_tags($branch_id) . "', '" . strip_tags($cus_count1) . "', 
		'" . strip_tags($loan_count1) . "','" . strip_tags($userid) . "',current_timestamp() )";
		$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);

		$last_id = $mysqli->insert_id;
		$deletQry = "DELETE FROM `area_line_mapping_area` WHERE line_map_id = $last_id ";
		$insresult = $mysqli->query($deletQry) or die("Error " . $mysqli->error);

		// Remove trailing comma and extra spaces
		$area_ids = rtrim($area_id, ',');
		$area_ids_array = explode(',', $area_ids);

		foreach ($area_ids_array as $area_id) {
			$insertQry = "INSERT INTO `area_line_mapping_area`( `line_map_id`, `area_id`, `created_date`) VALUES ('$last_id','$area_id',current_timestamp())
        ";
	...
	public function addAreaMappingDuefollowup($mysqli, $userid)
	{
		...
		$insertQry = "INSERT INTO area_duefollowup_mapping(duefollowup_name, cus_count, loan_count, company_id, branch_id, insert_login_id, created_date) VALUES('" . strip_tags($duefollowup_name) . "',  '" . strip_tags($cus_count) . "', '" . strip_tags($loan_count) . "', '" . strip_tags($company_id) . "','" . strip_tags($branch_id) . "', '" . strip_tags($userid) . "', CURRENT_TIMESTAMP() )";
		$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);
		...
		foreach ($area_ids_array as $area_id) {
			$insertQry = "INSERT INTO `area_duefollowup_mapping_area`( `map_id`, `area_id`, `created_date`) VALUES ('$last_id','$area_id',current_timestamp())
        ";
	...
	public function addAreaMappingGroup($mysqli, $userid)
	{
		...
		$insertQry = "INSERT INTO area_group_mapping(group_name,company_id, branch_id, cus_count, loan_count, insert_login_id, created_date)
		VALUES('" . strip_tags($group_name) . "','" . strip_tags($company_id) . "', '" . strip_tags($branch_id) . "',
		'" . strip_tags($cus_count2) . "', '" . strip_tags($loan_count2) . "', '" . strip_tags($userid) . "',current_timestamp() )";
		$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);

		$last_id = $mysqli->insert_id;
		$deletQry = "DELETE FROM `area_group_mapping_area` WHERE group_map_id = $last_id ";
		$insresult = $mysqli->query($deletQry) or die("Error " . $mysqli->error);
		...
		foreach ($area_ids_array as $area_id) {
			$insertQry = "INSERT INTO `area_group_mapping_area`( `group_map_id`, `area_id`, `created_date`) VALUES ('$last_id','$area_id',current_timestamp())
        ";
	...
	public function updateAreaMappingLine($mysqli, $id, $userid)
	{
		...
		$updateQry = "UPDATE area_line_mapping set line_name='" . strip_tags($line_name) . "',company_id='" . strip_tags($company_id) . "',
		branch_id= '" . strip_tags($branch_id) . "', cus_count='" . strip_tags($cus_count1) . "', loan_count='" . strip_tags($loan_count1) . "',
		update_login_id='" . strip_tags($userid) . "', updated_date = current_timestamp(), status=0 WHERE map_id = '" . $id . "' ";
		$result = $mysqli->query($updateQry) or die("Error " . $mysqli->error);

		$deletQry = "DELETE FROM `area_line_mapping_area` WHERE line_map_id = $id ";
		$insresult = $mysqli->query($deletQry) or die("Error " . $mysqli->error);
		...
		foreach ($area_ids_array as $area_id) {
			$insertQry = "INSERT INTO `area_line_mapping_area`( `line_map_id`, `area_id`, `created_date`) VALUES ('$id','$area_id',current_timestamp())
        ";
	...
	public function updateAreaMappingDuefollowup($mysqli, $id, $userid)
	{
		...
		$updateQry = "UPDATE area_duefollowup_mapping set duefollowup_name='" . strip_tags($duefollowup_name) . "', cus_count='" . strip_tags($cus_count) . "', loan_count='" . strip_tags($loan_count) . "', company_id='" . strip_tags($company_id) . "', branch_id= '" . strip_tags($branch_id) . "', update_login_id='" . strip_tags($userid) . "', updated_date = current_timestamp(), status=0 WHERE map_id = '" . $id . "' ";
		$result = $mysqli->query($updateQry) or die("Error " . $mysqli->error);
		...
		foreach ($area_ids_array as $area_id) {
			$insertQry = "INSERT INTO `area_duefollowup_mapping_area`( `map_id`, `area_id`, `created_date`) VALUES ('$id','$area_id',current_timestamp())
        ";
	...
	public function updateAreaMappingGroup($mysqli, $id, $userid)
	{
		...
		$updateQry = "UPDATE area_group_mapping set group_name='" . strip_tags($group_name) . "',company_id='" . strip_tags($company_id) . "',
		branch_id= '" . strip_tags($branch_id) . "', cus_count='" . strip_tags($cus_count2) . "',loan_count='" . strip_tags($loan_count2) . "',
		update_login_id='" . strip_tags($userid) . "', updated_date	 = current_timestamp(), status=0 WHERE map_id = '" . $id . "' ";
		$result = $mysqli->query($updateQry) or die("Error " . $mysqli->error);

		$deletQry = "DELETE FROM `area_group_mapping_area` WHERE group_map_id = $id ";
		$insresult = $mysqli->query($deletQry) or die("Error " . $mysqli->error);
		...
		foreach ($area_ids_array as $area_id) {
			$insertQry = "INSERT INTO `area_group_mapping_area`( `group_map_id`, `area_id`, `created_date`) VALUES ('$id','$area_id',current_timestamp())
        ";
	...
	{
		$selectQry = "SELECT alm.*,alma.area_id FROM area_line_mapping alm join area_line_mapping_area alma on alma.line_map_id = alm.map_id WHERE alm.map_id='" . mysqli_real_escape_string($mysqli, $id) . "' ";
		$res = $mysqli->query($selectQry) or die("Error in Get All Records" . $mysqli->error);
		$detailrecords = array();
		if ($mysqli->affected_rows > 0) {
			$area_ids = [];
	...
	{
		$selectQry = "SELECT adm.*,adma.area_id as area_id FROM area_duefollowup_mapping adm  join area_duefollowup_mapping_area adma on adma.map_id = adm.map_id WHERE adm.map_id='" . mysqli_real_escape_string($mysqli, $id) . "' ";
		$res = $mysqli->query($selectQry) or die("Error in Get All Records" . $mysqli->error);
		$detailrecords = array();
		if ($mysqli->affected_rows > 0) {
			$area_ids = [];
	...
	{
		$selectQry = "SELECT agm.*,agma.area_id FROM area_group_mapping agm join area_group_mapping_area agma on agma.group_map_id = agm.map_id WHERE agm.map_id='" . mysqli_real_escape_string($mysqli, $id) . "' ";
		$res = $mysqli->query($selectQry) or die("Error in Get All Records" . $mysqli->error);
		$detailrecords = array();
		if ($mysqli->affected_rows > 0) {
			$area_ids = [];
	...
	public function addUser($mysqli, $userid)
	{
		...
		if (isset($_POST['promotion_activity_mapping_access']) && $_POST['promotion_activity_mapping_access'] != '') {
			$promotion_activity_Access = $_POST['promotion_activity_mapping_access'];
		} else {
		...
		if (isset($_POST['conf_followup_line_or_duefollowup'])) {
			$conf_followup_Access = $_POST['conf_followup_line_or_duefollowup'];
		} else {
			$conf_followup_Access = '';
		}
		...
		if (isset($_POST['uncleared_report']) &&    $_POST['uncleared_report'] == 'Yes') {
			$uncleared_report = 0;
		} else {
			$uncleared_report = 1;
		}
		...
		if (isset($_POST['reportmodule_interest']) &&    $_POST['reportmodule_interest'] == 'Yes') {
			$reportmodule_interest = 0;
		} else {
			$reportmodule_interest = 1;
		}
		if (isset($_POST['interest_ledger_report']) &&    $_POST['interest_ledger_report'] == 'Yes') {
			$interest_ledger_report = 0;
		} else {
			$interest_ledger_report = 1;
		}
		if (isset($_POST['interest_loan_issue_report']) &&    $_POST['interest_loan_issue_report'] == 'Yes') {
			$interest_loan_issue_report = 0;
		} else {
			$interest_loan_issue_report = 1;
		}
		if (isset($_POST['interest_collection_report']) &&    $_POST['interest_collection_report'] == 'Yes') {
			$interest_collection_report = 0;
		} else {
			$interest_collection_report = 1;
		}
		if (isset($_POST['interest_balance_report']) &&    $_POST['interest_balance_report'] == 'Yes') {
			$interest_balance_report = 0;
		} else {
			$interest_balance_report = 1;
		}
		if (isset($_POST['interest_closed_report']) &&    $_POST['interest_closed_report'] == 'Yes') {
			$interest_closed_report = 0;
		} else {
			$interest_closed_report = 1;
		}
	...
	public function addDocumentMapping($mysqli, $userid)
	{
		...
		VALUES('" . strip_tags($loan_category) . "','" . strip_tags($doc_creation) . "','" . strip_tags($userid) . "','CURRENT_TIMESTAMP' )";
	...
	public function updateDocumentMapping($mysqli, $id, $userid)
	{
		...
		$insertQry = "UPDATE `doc_mapping` SET `loan_category`='" . strip_tags($loan_category) . "',`doc_creation`='" . strip_tags($doc_creation) . "',
	...
	public function addCustomerProfile($mysqli, $userid)
	{
		...
		if ($cus_Tableid == '') {
			...
			if ($qry->num_rows == 0) {
				...
				$insertQry = "UPDATE in_verification set agent_id = '" . strip_tags($cus_agent_name) . "' ,cus_status = 10,`cus_id`='" . strip_tags($cus_id) . "',`first_name`='" . strip_tags($first_name) . "', `last_name`='" . strip_tags($last_name) . "', `gender`='" . strip_tags($gender) . "',`dob`='" . strip_tags($dob) . "',`age`='" . strip_tags($age) . "',`blood_group`='" . strip_tags($bloodGroup) . "', `state`='" . strip_tags($state) . "',`district`='" . strip_tags($district) . "',`taluk`='" . strip_tags($taluk) . "',`area`='" . strip_tags($area) . "', `mobile1`='" . strip_tags($mobile1) . "', `mobile2`='" . strip_tags($mobile2) . "', `whatsapp_no`='" . strip_tags($whatsapp_no) . "', `pic`='" . strip_tags($pic_req) . "',updated_date=now() where req_id ='" . strip_tags($req_id) . "' ";
				$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);
			}
		} else {
	...
	function addVerificationLoanCalculation($mysqli, $userid)
	{
		...
		if ($loan_cal_id > 0 and $loan_cal_id != '') {
			$mysqli->query("UPDATE verification_loan_calculation SET cus_id_loan = '" . strip_tags($cus_id_loan) . "', first_name = '" . strip_tags($first_name) . "', 
			last_name = '" . strip_tags($last_name) . "', cus_data_loan = '" . strip_tags($cus_data_loan) . "', mobile_loan = '" . strip_tags($mobile_loan) . "', pic_loan = '" . strip_tags($pic_loan) . "', 
				loan_category = '" . strip_tags($loan_category) . "', tot_value = '" . strip_tags($tot_value) . "', ad_amt = '" . strip_tags($ad_amt) . "',
			...
			if ($qry->num_rows == 0) {
				...
				VALUES ('" . strip_tags($req_id) . "', '" . strip_tags($cus_id_loan) . "', '" . strip_tags($first_name) . "', '" . strip_tags($last_name) . "', 
				'" . strip_tags($cus_data_loan) . "','" . strip_tags($mobile_loan) . "', '" . strip_tags($pic_loan) . "', '" . strip_tags($loan_category) . "', '" . strip_tags($tot_value) . "', '" . strip_tags($ad_amt) . "', '" . strip_tags($loan_amt) . "', '" . strip_tags($profit_type) . "', 
		...
		try {
			if ($doc_table_id == '') {
				...
				if ($qry->num_rows == 0) {
					...
					$insertQry = "INSERT INTO `acknowlegement_documentation`( `req_id`, `cus_id_doc`, `first_name`, `last_name`, `cus_profile_id`, `mortgage_process`, `Propertyholder_type`, `Propertyholder_name`, `Propertyholder_relationship_name`, `doc_property_relation`, `doc_property_type`, `doc_property_measurement`, `doc_property_location`, `doc_property_value`, `endorsement_process`, `owner_type`, `owner_name`, `ownername_relationship_name`, `en_relation`, `vehicle_type`, `vehicle_process`, `en_Company`, `en_Model`, `cus_status`, `insert_login_id`,`created_date`) VALUES('" . strip_tags($req_id) . "','" . strip_tags($cus_id_doc) . "','" . strip_tags($first_name) . "', '" . strip_tags($last_name) . "', '" . strip_tags($cus_profile_id) . "','" . strip_tags($mortgage_process) . "', '" . strip_tags($Propertyholder_type) . "', '" . strip_tags($Propertyholder_name) . "','" . strip_tags($Propertyholder_relationship_name) . "','" . strip_tags($doc_property_relation) . "','" . strip_tags($doc_property_pype) . "','" . strip_tags($doc_property_measurement) . "', '" . strip_tags($doc_property_location) . "', '" . strip_tags($doc_property_value) . "', '" . strip_tags($endorsement_process) . "','" . strip_tags($owner_type) . "','" . strip_tags($owner_name) . "','" . strip_tags($ownername_relationship_name) . "','" . strip_tags($en_relation) . "','" . strip_tags($vehicle_type) . "','" . strip_tags($vehicle_process) . "','" . strip_tags($en_Company) . "','" . strip_tags($en_Model) . "','11','" . $userid . "',current_timestamp() )";
	...
	function addAcknowledgementLoanCalculation($mysqli, $userid)
	{
		...
		if ($loan_cal_id > 0 and $loan_cal_id != '') {
			$updateQry = $mysqli->query("UPDATE acknowlegement_loan_calculation SET cus_id_loan = '" . strip_tags($cus_id_loan) . "', 
						first_name = '" . strip_tags($first_name) . "', last_name = '" . strip_tags($last_name) . "', 
	...
	function getLoanList($mysqli, $id)
	{
		...
		$qry = $mysqli->query("SELECT b.branch_id, b.branch_name, l.map_id, l.line_name AS area_line FROM branch_creation b JOIN area_line_mapping l ON l.branch_id = b.branch_id join area_line_mapping_area alma on alma.line_map_id = l.map_id WHERE alma.area_id = " . $detailrecords['area_confirm_area'] . " ");
		$row = $qry->fetch_assoc();
	...
	function addConcern($mysqli, $userid)
	{
		...
		$insertQry = "INSERT INTO `concern_creation`( `raising_for`, `self_name`, `self_code`, `staff_name`, `staff_dept_name`, `staff_team_name`, `ag_name`, `ag_grp`, `cus_id`, `first_name`, `last_name`, `cus_area`,  `cus_group`, `cus_line`, `com_date`, `com_code`, `branch_name`, `concern_to`, `to_dept_name`, `to_team_name`, `com_remark`, `com_priority`, `staff_assign_to`, `insert_user_id`) VALUES('" . strip_tags($raising_for) . "','" . strip_tags($self_name) . "','" . strip_tags($self_code) . "','" . strip_tags($staff_name) . "',
				'" . strip_tags($staff_dept_name) . "', '" . strip_tags($staff_team_name) . "', '" . strip_tags($ag_name) . "', '" . strip_tags($ag_grp) . "','" . strip_tags($cus_id) . "', '" . strip_tags($first_name) . "', '" . strip_tags($last_name) . "','" . strip_tags($cus_area) . "', '" . strip_tags($cus_group) . "',
				'" . strip_tags($cus_line) . "', 
				'" . strip_tags($com_date) . "','" . strip_tags($com_code) . "','" . strip_tags($branch_name) . "','" . strip_tags($concern_to) . "','" . strip_tags($to_dept_name) . "','" . strip_tags($to_team_name) . "','" . strip_tags($com_remark) . "','" . strip_tags($com_priority) . "','" . strip_tags($staff_assign_to) . "','" . strip_tags($userid) . "')";
	...
	public function updateCustomerProfile($mysqli, $userid)
	{
		...
		$insertQry = "UPDATE in_verification set `first_name`='" . strip_tags($first_name) . "', `last_name`='" . strip_tags($last_name) . "', `gender`='" . strip_tags($gender) . "',`dob`='" . strip_tags($dob) . "',`age`='" . strip_tags($age) . "' ,`state`='" . strip_tags($state) . "',`district`='" . strip_tags($district) . "',`taluk`='" . strip_tags($taluk) . "',`area`='" . strip_tags($area) . "',`address`='" . strip_tags($cus_address) . "',`mobile1`='" . strip_tags($mobile1) . "', `mobile2`='" . strip_tags($mobile2) . "',`father_name`='" . strip_tags($father_name) . "',`mother_name`='" . strip_tags($mother_name) . "',`marital`='" . strip_tags($marital) . "',`spouse_name`='" . strip_tags($spouse_name) . "',`occupation_type`='" . strip_tags($occupation_type) . "',`occupation`='" . strip_tags($occupation) . "',`pic` = '" . strip_tags($cus_pic) . "' where `cus_id`='" . strip_tags($cus_id) . "' ";
		$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);
		...
		if (!empty($req_id_array)) {
			$cusUpd = "UPDATE `customer_profile` SET `cus_id`='" . strip_tags($cus_id) . "',`first_name`='" . strip_tags($first_name) . "', `last_name`='" . strip_tags($last_name) . "',`gender`='" . strip_tags($gender) . "',`dob`='" . strip_tags($dob) . "',`mobile1`='" . strip_tags($mobile1) . "', `mobile2`='" . strip_tags($mobile2) . "',`age`='" . strip_tags($age) . "',`cus_pic`='" . strip_tags($cus_pic) . "',`residential_type`='" . strip_tags($cus_res_type) . "',`residential_details`='" . strip_tags($cus_res_details) . "',`residential_address`='" . strip_tags($cus_res_address) . "',`residential_native_address`='" . strip_tags($cus_res_native) . "',`occupation_type`='" . strip_tags($cus_occ_type) . "',`occupation_details`='" . strip_tags($cus_occ_detail) . "',`occupation_income`='" . strip_tags($cus_occ_income) . "',`occupation_address`='" . strip_tags($cus_occ_address) . "',`dow`='" . strip_tags($cus_occ_dow) . "',`abt_occ`='" . strip_tags($cus_occ_abt) . "',`area_confirm_type`='" . strip_tags($area_cnfrm) . "',`area_confirm_state`='" . strip_tags($area_state) . "',`area_confirm_district`='" . strip_tags($area_district) . "',`area_confirm_taluk`='" . strip_tags($area_taluk) . "',`area_confirm_area`='" . strip_tags($area_confirm) . "',`latlong`='" . strip_tags($latlong) . "',`area_group`='" . strip_tags($area_group) . "',`area_line`='" . strip_tags($area_line) . "',`update_login_id`='" . $userid . "',`updated_date`= now() WHERE req_id IN (" . implode(",", $req_id_array) . " ) ";
			$updateCus = $mysqli->query($cusUpd) or die("Error " . $mysqli->error);

			$insertQry = "UPDATE `request_creation` SET `first_name`='" . strip_tags($first_name) . "', `last_name`='" . strip_tags($last_name) . "',`dob`='" . strip_tags($dob) . "',`age`='" . strip_tags($age) . "', `gender`='" . strip_tags($gender) . "',`state`='" . strip_tags($state) . "',`district`='" . strip_tags($district) . "',
				`taluk`='" . strip_tags($taluk) . "',`area`='" . strip_tags($area) . "',`address`='" . strip_tags($cus_address) . "',`mobile1`='" . strip_tags($mobile1) . "',`mobile2`='" . strip_tags($mobile2) . "',`father_name`='" . strip_tags($father_name) . "',
				`mother_name`='" . strip_tags($mother_name) . "',`marital`='" . strip_tags($marital) . "',`spouse_name`='" . strip_tags($spouse_name) . "',`occupation_type`='" . strip_tags($occupation_type) . "',
				`occupation`='" . strip_tags($occupation) . "',`pic`='" . strip_tags($cus_pic) . "',
				`update_login_id`='" . strip_tags($userid) . "',`updated_date`=current_timestamp WHERE req_id IN (" . implode(",", $req_id_array) . " )";

			$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);


			$updateACkCus = "UPDATE `acknowlegement_customer_profile` SET `cus_id`='" . strip_tags($cus_id) . "',`first_name`='" . strip_tags($first_name) . "', `last_name`='" . strip_tags($last_name) . "',`gender`='" . strip_tags($gender) . "',`dob`='" . strip_tags($dob) . "',`age`='" . strip_tags($age) . "',`cus_pic`='" . strip_tags($cus_pic) . "',`residential_type`='" . strip_tags($cus_res_type) . "',`residential_details`='" . strip_tags($cus_res_details) . "',`residential_address`='" . strip_tags($cus_res_address) . "',`residential_native_address`='" . strip_tags($cus_res_native) . "',`occupation_type`='" . strip_tags($cus_occ_type) . "',`occupation_details`='" . strip_tags($cus_occ_detail) . "',`occupation_income`='" . strip_tags($cus_occ_income) . "',`occupation_address`='" . strip_tags($cus_occ_address) . "',`dow`='" . strip_tags($cus_occ_dow) . "',`abt_occ`='" . strip_tags($cus_occ_abt) . "',`area_confirm_type`='" . strip_tags($area_cnfrm) . "',`area_confirm_state`='" . strip_tags($area_state) . "',`area_confirm_district`='" . strip_tags($area_district) . "',`area_confirm_taluk`='" . strip_tags($area_taluk) . "',`area_confirm_area`='" . strip_tags($area_confirm) . "',`latlong`='" . strip_tags($latlong) . "',`area_group`='" . strip_tags($area_group) . "',`area_line`='" . strip_tags($area_line) . "',`update_login_id`='" . $userid . "',`updated_date`= now() WHERE req_id IN (" . implode(",", $req_id_array) . " ) ";
	...
	public function sendfestivalSMS($mysqli, $userid)
	{
		...
		$qry = $mysqli->query("SELECT CONCAT(first_name, ' ', last_name) AS customer_name, mobile1 from customer_register ");
		while ($row = $qry->fetch_assoc()) {
