  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        var selected = (guarentor_name != "" && guarentor_name == fam_id) ? "selected" : "";
...
function propertyHolder() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      // Build HTML options
      var htmlString = "<option value=''>Select Property Holder</option>";
      for (var i = 0; i < response.length; i++) {
        var fam_name = response[i];
        htmlString += "<option value='" + fam_name + "'>" + fam_name + "</option>";
      }
...
function getDistrictDropdown(StateSelected) {
  ...
  var TamilNadu = [
    ...
    "Kancheepuram", "Kanniyakumari", "Karur", "Madurai", "Nagapattinam",
    "Namakkal", "Nilgiris", "Perambalur", "Pudukottai", "Ramanathapuram",
    "Salem", "Sivagangai", "Thanjavur", "Theni", "Thiruvallur",
    "Tiruvannamalai", "Thiruvarur", "Thoothukudi", "Tiruchirappalli",
    "Thirunelveli", "Vellore", "Viluppuram", "Virudhunagar", "Ariyalur",
    "Krishnagiri", "Tiruppur", "Chengalpattu", "Kallakurichi", "Ranipet",
    "Tenkasi", "Tirupathur", "Mayiladuthurai"
  ];
...
function getTalukDropdown(DistSelected) {
  ...
  // Sort optionsList alphabetically
  optionsList.sort(function(a, b) {
    return a.localeCompare(b);
  });
...
function getTalukBasedArea(talukselected) {
  ...
  $.ajax({
    ...
    dataType: "json",
    success: function (response) {
      // Sort response array alphabetically by area_name
      response.sort(function(a, b) {
...
function getStaffBasedAgent(user_id_load) {
  ...
  $.ajax({
    ...
    cache: false,
    success: function (response) {
      // Sort response array alphabetically by ag_name
      response.sort(function(a, b) {
        return a.ag_name.localeCompare(b.ag_name);
      });
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        $("#cus_agent_name").append(
          "<option value='" + response[i]["ag_id"] + "' " + selected + ">" + response[i]["ag_name"] + "</option>"
        );
      }
    },
  });
...
function getAllAgentDropdown() {
  ...
  $.ajax({
    ...
    cache: false,
    success: function (response) {
      // Sort the response array alphabetically by ag_name
      response.sort(function(a, b) {
        return a.ag_name.localeCompare(b.ag_name);
      });
  ...
  $.ajax({
    ...
    success: function (response) {
...

      {
        ...
        $("#ownername_relationship_name").html(
          $("#ownername_relationship_name option:not(:first-child)").sort(
            function (a, b) {
  ...
  $.ajax({
    ...
    success: function (response) {
...

      {
        //To Order ag_group Alphabetically
        var firstOption = $(
          "#Propertyholder_relationship_name option:first-child"
        );
        $("#Propertyholder_relationship_name").html(
          $("#Propertyholder_relationship_name option:not(:first-child)").sort(
            function (a, b) {
              return a.text == b.text ? 0 : a.text < b.text ? -1 : 1;
            }
          )
    ...
    success: function (response) {
      if (response["advance"] == "Yes") {
       ...
       $('#tot_value').on('input', function () {
            ...
            var amt = $("#tot_value").val().replace(/,/g, '') || 0;
            var advance = $("#ad_amt").val().replace(/,/g, '') || 0;
