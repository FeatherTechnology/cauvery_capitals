...
$(document).ready(function () {
    ...
    if (pageView == '1') {
        ...
        setTimeout(() => {
            getCategoryInfo();
            var loan_cal_id = $('#loan_cal_ids').val();
            getLoaninfo(loan_cal_id);
...
function resetFamDetails() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < len - 1; i++) {  // Keep your existing len-1 condition
...
function getDistrictDropdown(StateSelected) {
    ...
    // No need to sort again in DOM, already sorted
}
...
function getTalukBasedArea(talukselected) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Extract area names and sort alphabetically first
            ...
            for (var i = 0; i < sortedResponse.length; i++) {
                var area_id = sortedResponse[i]['area_id'];
                var area_name = sortedResponse[i]['area_name'];
                var selected = '';
...
function endorseHolderName() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var len = response.length;
            var htmlString = "<option value=''>" + 'Select Holder Name' + "</option>";

            // Sort the response alphabetically first
            var sortedResponse = response.slice(0, len - 1).sort(function(a, b) {
                return a.fam_name.localeCompare(b.fam_name);
            });
            ...
            for (var i = 0; i < sortedResponse.length; i++) {
                ...
                htmlString += "<option value='" + fam_id + "'>" + fam_name + "</option>";
            }

            $("#ownername_relationship_name").html(htmlString);
        }
    });
}
...
function mortgageHolderName() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#Propertyholder_relationship_name").html(htmlString);
        }
    });
}
...
function docHolderName() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#docholder_relationship_name").html(htmlString);
        }
    });
}
...
function signTypeRelation() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var len = response.length;
            var htmlString = "<option value=''>" + 'Select Relationship' + "</option>";

            // Sort the response alphabetically first by fam_name
            var sortedResponse = response.slice(0, len - 1).sort(function(a, b) {
                return a.fam_name.localeCompare(b.fam_name);
            });
            ...
            for (var i = 0; i < sortedResponse.length; i++) {
                var fam_name = sortedResponse[i]['fam_name'];
                var fam_id = sortedResponse[i]['fam_id'];
                var relationship = sortedResponse[i]['relationship'];
                htmlString += "<option value='" + fam_id + "'>" + fam_name + ' - ' + relationship + "</option>";
            }

            $("#signType_relationship").html(htmlString);
        }
    });
}
...
function chequeHolderName() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Sort the response alphabetically by fam_name before appending
            var sortedResponse = response.slice(0, len - 1).sort(function(a, b) {
                return a.fam_name.localeCompare(b.fam_name);
            });
            ...
            $("#holder_relationship_name").html(htmlString);
        }
    });
}
