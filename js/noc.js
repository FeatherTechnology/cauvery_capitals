    ...
    var req_id = $('#req_id').val();
        ...
        if (noc_member > 1) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (noc_member == 2) {
                        ...
                        if (response['fingers'] && response['fingers'].length > 0) {
                            // Multiple fingerprints supported
                            $('#compare_finger').val(JSON.stringify(response['fingers']));
                        ...
                        if (response['fam_id'] && response['fam_id'].length > 0) {
            ...
            $.ajax({
                ...
                error: function () {
                    alert('Failed to load customer fingerprints.');
                }
            });
        } else {
            // None selected
    ...
    $('#mem_relation_name').change(function () {
        ...
        if (fam_id != '') {
            $.ajax({
                ...
                error: function () {
                    alert('Failed to load family member fingerprints.');
                }
            });
        } else {
            $('#compare_finger').val('');
...
function updateCheckedDetails() {
    ...
    sign_deffer.done(function () {
        ...
        $.ajax({
            ...
            data: { 'noc_details': sign_check, 'table_name': 'signed_doc_info', sign_ids, req_id },
            type: 'post',
            cache: false,
            success: function (response) {
