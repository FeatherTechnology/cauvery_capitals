...
$qry = $connect->query("
    SELECT 
        ...
        (SELECT bal_amt - (princ_amt_track + principal_waiver) as bal_amt FROM collection where req_id = cp.req_id and (month(date(coll_date)) <= month('$monthly_date') 
    ...
    WHERE 
        (ii.cus_status >= 14 && ii.cus_status < 20) AND lc.due_type = 'Interest' AND lc.calc_method = 'Monthly' AND li.balance_amount = 0
            ...
            buttons: [{
                    ...
                    action: function(e, dt, node, config) {
                        ...
                        const {
                            title,
                            filename
                        } = generateReportTitle('Interest Loan - Ledger View Report Monthly List');
