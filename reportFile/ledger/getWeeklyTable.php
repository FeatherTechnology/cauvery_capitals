...
$qry = $connect->query("
    ...
    WHERE 
        (ii.cus_status >= 14 && ii.cus_status < 20) AND lc.due_method_scheme = 2 and date(lc.due_start_from) = '$weekly_date' $user_based GROUP BY cp.req_id");
...
<table class="table custom-table" id="weekly_table">
    ...
    <tfoot>
        <?php
        $tfoot = "<tr><td colspan='7'><b>Total</b></td><td><b>" . moneyFormatIndia($due_amt_sum) . "</b></td><td><b>" . moneyFormatIndia($opening_balance_sum) . "</b></td><td colspan=" . $total_weeks . "></td><td><b>" . moneyFormatIndia($total_paid_sum) . "</b></td><td><b>" . moneyFormatIndia($closing_balance_sum) . "</b></td></tr>";
        echo $tfoot;
        ?>
    </tfoot>
</table>
