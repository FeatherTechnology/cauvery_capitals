...
if (isset($_POST['branchid']) && isset($_POST['status'])) {
    ...
    $qry = $connect->query("SELECT adma.area_id FROM area_duefollowup_mapping_area adma LEFT JOIN area_duefollowup_mapping adm ON adma.map_id = adm.map_id  WHERE `branch_id` = $branchid AND `customer_status` IN ($statusList)");
    $excludeAreaIds = [];
    if($qry->rowCount() > 0){
